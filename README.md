...
6. 战斗系统：将角色的持续性数据（如最大生命值）和当前属性（如当前生命值）分离，并将数据统一管理，便于其它系统对战斗属性的访问和修改；实现了战斗的**碰撞检测**以及玩家和敌人基础的攻击、受击和死亡动画反馈。实现战斗时对敌人的**镜头锁定**，并根据敌我距离动态调整镜头距离。
...
9. 相机管理：采用**Cinemachine**中的Third Person Aim Camera控制第三人称相机的跟随，同时自定义控制相机的旋转和战斗、探索时相机模式的切换。
+ 游戏截图
